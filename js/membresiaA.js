$(document).ready(function() {
	...
	$("form#formMemb").validetta({
			...
			bubblePosition: 'bottom',
  			...
  			onValid : function(e){
	  			e.preventDefault();
	  			var obj=$("form#formMemb").serializeToJSON({
	  				associativeArrays: true,
				});
				var jsonString = JSON.stringify(obj);
		        	$.ajax({
			            method:"post",
			            url:"php/membresia.php",
			                ...
			                if(Jresp["estado"]=="1"){
			                    $.alert({
			                        ...
			                        onDestroy: function () {
		    							...
		    							$('#formMemb').find('input, textarea, button, select').attr('disabled','disabled');
		    							$('#formMemb').find('.fa-plus').hide();
		    							$('#formMemb').find('.fa-minus').hide();
	    							}
			                    });
			                }  
			            },
